...
{
    public partial class observeStudyHelpsForm2 : Form
    {
        string connection = "Data Source=DESKTOP-S1F0LH1;Initial Catalog=kheirie;Integrated Security=True";
        string id;
        public observeStudyHelpsForm2(string id)
        {
            InitializeComponent();
            this.id = id;
        }

        private void observeStudyHelpsForm2_Load(object sender, EventArgs e)
        {
            ...
            con.Open();
            SqlCommand cmdcheck = new SqlCommand("select type from StudyHelps where id = @id;", con);
            cmdcheck.Parameters.AddWithValue("@id", this.id);
            using(SqlDataReader reader = cmdcheck.ExecuteReader())
            {
                if (reader.Read())
                {
                    if(reader.GetString(0) == "معرفی‌نامه")
                    {
                        chooseButton.Text = "معرفی‌نامه";
                    }
                    else
                    {
                        chooseButton.Text = "رسید";
                    }
                }
            }
            SqlCommand cmd2; SqlDataAdapter da; DataTable dt;
            // tmp
            cmd2 = new SqlCommand("select hId as 'شماره کمک', supporter_id as 'شماره ملی سرپرست', stuId as 'شماره ملی', folder_id as 'شماره پرونده', name as 'نام', family as 'نام خانودگی' from StudyTmpList where hId = @id;", con);
            ...
            // fin
            cmd2 = new SqlCommand("select hId as 'شماره کمک', supporter_id as 'شماره ملی سرپرست', stuId as 'شماره ملی', folder_id as 'شماره پرونده', name as 'نام', family as 'نام خانودگی' from StudyFinList where hId = @id;", con);
            cmd2.Parameters.AddWithValue("@id", this.id);
            da = new SqlDataAdapter(cmd2);
            dt = new DataTable();
            da.Fill(dt);
            ...
            // del
            cmd2 = new SqlCommand("select hId as 'شماره کمک', supporter_id as 'شماره ملی سرپرست', stuId as 'شماره ملی', folder_id as 'شماره پرونده', name as 'نام', family as 'نام خانودگی' from StudyDelList where hId = @id;", con);
            cmd2.Parameters.AddWithValue("@id", this.id);
            da = new SqlDataAdapter(cmd2);
            dt = new DataTable();
            da.Fill(dt);
            ...
            // notcome
            cmd2 = new SqlCommand("select hId as 'شماره کمک', supporter_id as 'شماره ملی سرپرست', stuId as 'شماره ملی', folder_id as 'شماره پرونده', name as 'نام', family as 'نام خانودگی' from StudyNotComeList where hId = @id;", con);
            cmd2.Parameters.AddWithValue("@id", this.id);
            da = new SqlDataAdapter(cmd2);
            dt = new DataTable();
            da.Fill(dt);
            ...
            // rec
            cmd2 = new SqlCommand("select hId as 'شماره کمک', supporter_id as 'شماره ملی سرپرست', stuId as 'شماره ملی', folder_id as 'شماره پرونده', name as 'نام', family as 'نام خانودگی' from StudyRecList where hId = @id;", con);
            cmd2.Parameters.AddWithValue("@id", this.id);
            da = new SqlDataAdapter(cmd2);
            dt = new DataTable();
            da.Fill(dt);
        ...
        private void chooseButton_Click(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(this.connection);
            con.Open();
            SqlCommand cmd = new SqlCommand("select docpath from doc where id = @id and doctype = 'Shelp';", con);
