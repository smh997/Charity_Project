...
{
    public partial class observebuyhelpsForm2 : Form
    {
        string connection = "Data Source=DESKTOP-S1F0LH1;Initial Catalog=kheirie;Integrated Security=True";
        string id;
        public observebuyhelpsForm2(string id)
        {
            InitializeComponent();
            this.id = id;
        }

        private void observebuyhelpsForm2_Load(object sender, EventArgs e)
        {
            receipeButton.Enabled = chooseButton.Enabled = exportButton2.Enabled = false;
            SqlConnection con = new SqlConnection(this.connection);
            con.Open();
            SqlCommand cmd2; SqlDataAdapter da; DataTable dt;
            cmd2 = new SqlCommand("select id as 'شماره کمک', type as 'نوع کمک', fee as 'ارزش ریالی', packet as 'تعداد بسته', dbo.MiladiTOShamsi(buydate) as 'تاریخ خرید', dbo.MiladiTOShamsi(subdate) as 'تاریخ ثبت', buyerName as 'نام خریدار', bankAccountName as 'نام حساب', bankAccountId as 'شماره حساب', description as 'توضیحات', enactmentId as 'شماره مصوبه' from buyHelpsHistory where id = @id;", con);
        ...
        {
            receipeButton.Enabled = chooseButton.Enabled = exportButton2.Enabled = (membersView.SelectedCells.Count != 0);
        }
        ...
        private void receipeButton_Click(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(this.connection);
            con.Open();
            SqlCommand cmd = new SqlCommand("select docpath from doc where id = @id and doctype = 'buyHelp';", con);
