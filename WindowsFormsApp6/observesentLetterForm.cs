...
{
    public partial class observesentLetterForm : Form
    {
        string connection = "Data Source=DESKTOP-S1F0LH1;Initial Catalog=kheirie;Integrated Security=True";
        public observesentLetterForm()
        {
            InitializeComponent();
        }

        private void observesentLetterForm_Load(object sender, EventArgs e)
        {
        ...
        private void searchButton_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand("select id as 'شماره نامه', title as 'عنوان', receiver as 'گیرنده', signer as 'امضاکننده', attachment as 'پیوست', dbo.MiladiTOShamsi(senddate) as 'تاریخ ارسال', dbo.MiladiTOShamsi(subdate) as 'تاریخ ثبت' from sendingLetters where senddate >= '" + startDateTimePickerX.SelectedDateInStringEnglish + "' and senddate <= '" + endDateTimePickerX.SelectedDateInStringEnglish + "';", con1);
        ...
        {
            var newform = new observesentLetterForm2(ExtensionFunction.PersianToEnglish(membersView.Rows[membersView.SelectedCells[0].RowIndex].Cells[membersView.SelectedCells[0].ColumnIndex].Value.ToString()));
