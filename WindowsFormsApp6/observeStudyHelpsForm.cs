...
{
    public partial class observeStudyHelpsForm : Form
    {
        string connection = "Data Source=DESKTOP-S1F0LH1;Initial Catalog=kheirie;Integrated Security=True";
        public observeStudyHelpsForm()
        {
            InitializeComponent();
        }

        private void observeStudyHelpsForm_Load(object sender, EventArgs e)
        {
            chooseButton.Enabled = exportButton2.Enabled = false;
            SqlConnection con = new SqlConnection(this.connection);
            con.Open();
            SqlCommand cmd2; SqlDataAdapter da; DataTable dt;
            cmd2 = new SqlCommand("select id as 'شماره کمک', type as 'نوع کمک', dbo.MiladiTOShamsi(subdate) as 'تاریخ ثبت', dbo.MiladiTOShamsi(startdate) as 'تاریخ شروع', dbo.MiladiTOShamsi(enddate) as 'تاریخ پایان', school as 'نام مدرسه', cast(fee as decimal(15,0)) as 'ارزش ریالی', number as 'تعداد', status as 'وضعیت', enactmentId as 'شماره مصوبه تعریف', fenactmentId as 'شماره مصوبه تایید', description as 'توضیحات کمک' from StudyHelps;", con);
        ...
        private void searchButton_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand("select id as 'شماره کمک', type as 'نوع کمک', dbo.MiladiTOShamsi(subdate) as 'تاریخ ثبت', dbo.MiladiTOShamsi(startdate) as 'تاریخ شروع', dbo.MiladiTOShamsi(enddate) as 'تاریخ پایان', school as 'نام مدرسه', cast(fee as decimal(15,0)) as 'ارزش ریالی', number as 'تعداد', status as 'وضعیت', enactmentId as 'شماره مصوبه تعریف', fenactmentId as 'شماره مصوبه تایید', description as 'توضیحات کمک' from StudyHelps where " + sss + ";", con1);
        ...
        {
            var newform = new observeStudyHelpsForm2(ExtensionFunction.PersianToEnglish(membersView.Rows[membersView.SelectedCells[0].RowIndex].Cells[membersView.SelectedCells[0].ColumnIndex].Value.ToString()));
        ...
        {
            presCheckBox.Enabled = toolCheckBox.Checked;
            if (!presCheckBox.Enabled)
            {
