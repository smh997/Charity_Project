...
{
    public partial class observeOtherIndivReqsForm2 : Form
    {
        string connection = "Data Source=DESKTOP-S1F0LH1;Initial Catalog=kheirie;Integrated Security=True";
        string id;
        public observeOtherIndivReqsForm2(string id)
        {
            InitializeComponent();
            this.id = id;
        }

        private void observeOtherIndivReqsForm2_Load(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(this.connection);
            con.Open();
            SqlCommand cmd2; SqlDataAdapter da; DataTable dt;
            cmd2 = new SqlCommand("select id as 'شماره درخواست کمک', mId as 'شماره ملی متقاضی', status as 'وضعیت', dbo.MiladiTOShamsi(subdate) as 'تاریخ ثبت', dbo.MiladiTOShamsi(reqdate) as 'تاریخ درخواست', enactmentId as 'شماره مصوبه', description as 'توضیحات' from OtherHelpIndivReq where id = @id;", con);
        ...
        private void reqButton_Click(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(this.connection);
            con.Open();
            SqlCommand cmd = new SqlCommand("select docpath from doc where id = @id and doctype = 'OIRhelp:reqForm';", con);
        ...
        {
            var newform = new observeDocForm("متفرقه", "OIRhelp:otherDoc", this.id);
