...
{
    public partial class healHelpPresentForm : Form
    {
        string connection = "Data Source=DESKTOP-S1F0LH1;Initial Catalog=kheirie;Integrated Security=True";
        string helpPath = "C:\\Users\\hashemi\\Desktop\\Kheirie warehouse\\helps";
        string id, mId, explain, RF, prebId;
        decimal fee;
        List<KeyValuePair<string, string>> li;
        public healHelpPresentForm(string id, string p = "")
        {
            InitializeComponent();
        ...
        private void healHelpPresentForm_Load(object sender, EventArgs e)
        {
            ...
            if (this.Text == "ارائه کمک درمان")
            {
                feeTextBox.Visible = label3.Visible = true;
                cmdget = new SqlCommand("select mId, HealHelps.description, HealHelpReq.description, fee from HealHelps, HealHelpReq where HealHelps.reqId = HealHelpReq.id and HealHelps.id = @id;", con);
            ...
            else
            {
                ...
                feeTextBox.Visible = label3.Visible = true;
                cmdget = new SqlCommand("select mId, HealHelps.description, HealHelpReq.description, presdescription, fee, bankAccountId, bankAccountName from HealHelps, HealHelpReq where HealHelps.reqId = HealHelpReq.id and HealHelps.id = @id;", con);
                cmdget.Parameters.AddWithValue("@id", this.id);
                using (SqlDataReader reader = cmdget.ExecuteReader())
                {
                    if (reader.Read())
        ...
        private void delButton_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("BEGIN TRY begin tran t1; update HealHelps Set presdescription = Null, enddate = Null, status = @stat where id = @id; update bankAccount set stock = @st where id = @bId; delete from doc where id = @id and doctype = @dtype; commit tran t1; END TRY BEGIN CATCH ROLLBACK END CATCH", con);
            cmd.Parameters.AddWithValue("@id", this.id);
            cmd.Parameters.AddWithValue("@bId", this.prebId);
            cmd.Parameters.AddWithValue("@st", prestock + fee);
            cmd.Parameters.AddWithValue("@stat", "ارائه");
            cmd.Parameters.AddWithValue("@dtype", "Hhelp:receipt");
            cmd.ExecuteNonQuery();
            con.Close();
        ...
        private void setButton_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                DialogResult res = FMessegeBox.FarsiMessegeBox.Show("آیا نسبت به ویرایش ارائه این کمک اطمینان دارید؟", "پرسش", FMessegeBox.FMessegeBoxButtons.YesNo, FMessegeBox.FMessegeBoxIcons.Question, FMessegeBox.FMessegeBoxDefaultButton.button1);
                if (res != DialogResult.Yes)
                {
                    return;
                }
                ...
                if (receiptLabel.BackColor == Color.GreenYellow)
                {
                    ...
                    string destFile = System.IO.Path.Combine(targetPath, fileName);
                    System.IO.File.Copy(addOpenFileDialog.FileName, destFile, false);
                    SqlCommand cmd = new SqlCommand("BEGIN TRY begin tran t1; update HealHelps Set presdescription = @des, bankAccountName = @bName, bankAccountId = @bId where id = @id; update doc Set docname = @dname, docpath = @dpath, subdate = @edate where id = @id and doctype = @dtype; commit tran t1; END TRY BEGIN CATCH ROLLBACK END CATCH", con);
                    cmd.Parameters.AddWithValue("@id", this.id);
                ...
                {
                    SqlCommand cmd = new SqlCommand("update HealHelps Set presdescription = @des, bankAccountName = @bName, bankAccountId = @bId where id = @id;", con);
                    cmd.Parameters.AddWithValue("@id", this.id);
                    cmd.Parameters.AddWithValue("@bName", bankAccountNameComboBox.Text);
                    cmd.Parameters.AddWithValue("@bId", ExtensionFunction.PersianToEnglish(bankAccountNumberTextBox.Text));
                    cmd.Parameters.AddWithValue("@des", this.explain + "(توضیحات ویرایش ارائه: " + explainTextBox2.Text + ")");
