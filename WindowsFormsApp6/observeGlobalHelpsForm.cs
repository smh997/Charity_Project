...
{
    public partial class observeGlobalHelpsForm : Form
    {
        string connection = "Data Source=DESKTOP-S1F0LH1;Initial Catalog=kheirie;Integrated Security=True";
        public observeGlobalHelpsForm()
        {
            InitializeComponent();
        }

        private void observeGlobalHelpsForm_Load(object sender, EventArgs e)
        {
            chooseButton.Enabled = exportButton2.Enabled = false;
            SqlConnection con = new SqlConnection(this.connection);
            con.Open();
            SqlCommand cmd2; SqlDataAdapter da; DataTable dt;
            cmd2 = new SqlCommand("select id as 'شماره کمک', defType as 'نوع تعریف', type as 'نوع کمک', dbo.MiladiTOShamsi(subdate) as 'تاریخ ثبت', dbo.MiladiTOShamsi(startdate) as 'تاریخ شروع', dbo.MiladiTOShamsi(enddate) as 'تاریخ پایان', cast(fee as decimal(15,0)) as 'ارزش ریالی', packets as 'تعداد بسته', metric as 'مبلغ معیار امتیاز', status as 'وضعیت', enactmentId as 'شماره مصوبه تعریف', fenactmentId as 'شماره مصوبه تایید', description as 'توضیحات' from GlobalHelps;", con);
        ...
        private void searchButton_Click(object sender, EventArgs e)
        {
            ...
            foreach (CheckBox ch in enactmentGroupBox.Controls)
            {
                if (ch.Checked)
                {
                    if (ss2 != "")
            ...
            cmd = new SqlCommand("select id as 'شماره کمک', defType as 'نوع تعریف', type as 'نوع کمک', dbo.MiladiTOShamsi(subdate) as 'تاریخ ثبت', dbo.MiladiTOShamsi(startdate) as 'تاریخ شروع', dbo.MiladiTOShamsi(enddate) as 'تاریخ پایان', cast(fee as decimal(15,0)) as 'ارزش ریالی', packets as 'تعداد بسته', metric as 'مبلغ معیار امتیاز', status as 'وضعیت', enactmentId as 'شماره مصوبه تعریف', fenactmentId as 'شماره مصوبه تایید', description as 'توضیحات' from GlobalHelps where " + sss + ";", con1);
        ...
        {
            var newform = new observeGlobalHelpsForm2(ExtensionFunction.PersianToEnglish(membersView.Rows[membersView.SelectedCells[0].RowIndex].Cells[membersView.SelectedCells[0].ColumnIndex].Value.ToString()));
