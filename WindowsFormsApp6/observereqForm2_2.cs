...
{
    public partial class observereqForm2_2 : Form
    {
        string connection = "Data Source=DESKTOP-S1F0LH1;Initial Catalog=kheirie;Integrated Security=True";
        string id;
        public observereqForm2_2(string id)
        {
            InitializeComponent();
            this.id = id;
        }

        private void observereqForm2_2_Load(object sender, EventArgs e)
        {
            exportButton2.Enabled = false;
            SqlConnection con = new SqlConnection(this.connection);
            con.Open();
            SqlCommand cmd2; SqlDataAdapter da; DataTable dt;
            cmd2 = new SqlCommand("select id as 'شماره تقاضا', applicantId as 'شماره ملی متقاضی', fullname as 'نام و نام خانوادگی', reqType as 'نوع تقاضا', dbo.MiladiTOShamsi(subdate) as 'تاریخ ثبت تقاضا', dbo.MiladiTOShamsi(checkdate) as 'تاریخ بررسی', result as 'نتیجه بررسی', description as 'توضیحات تقاضا', description2 as 'توضیحات بررسی' from request where applicantId = sup and result is not Null and deliveryDate is Null and id = @id;", con);
