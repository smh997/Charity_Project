...
{
    public partial class observeMoneyTransferForm2 : Form
    {
        string connection = "Data Source=DESKTOP-S1F0LH1;Initial Catalog=kheirie;Integrated Security=True";
        string id;
        public observeMoneyTransferForm2(string id)
        {
            InitializeComponent();
            this.id = id;
        }

        private void observeMoneyTransferForm2_Load(object sender, EventArgs e)
        {
            receipeButton.Enabled = exportButton2.Enabled = false;
            SqlConnection con = new SqlConnection(this.connection);
            con.Open();
            SqlCommand cmd2; SqlDataAdapter da; DataTable dt;
            cmd2 = new SqlCommand("select id as 'شماره تراکنش', srcid as 'شماره حساب مبدا', dstid as 'شمار حساب مقصد', dbo.MiladiTOShamsi(subdate) as 'تاریخ ثبت', dbo.MiladiTOShamsi(transferdate) as 'تاریخ انتقال', cast(fee as decimal(15,0)) as 'ارزش ریالی', cast(fee as decimal(15,0)) as 'کارمزد', transporterName as 'انتقال‌دهنده', enactmentId as 'شماره مصوبه', description as 'توضیحات' from moneyTransferHistory where id = @id;", con);
        ...
        {
            receipeButton.Enabled = exportButton2.Enabled = (membersView.SelectedCells.Count != 0);
        }
        ...
        private void receipeButton_Click(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(this.connection);
            con.Open();
            SqlCommand cmd = new SqlCommand("select docpath from doc where id = @id and doctype = 'moneyTransfer';", con);
        ...
        private void chooseButton_Click(object sender, EventArgs e)
        {
            ...
            cmd.Parameters.AddWithValue("@id", ExtensionFunction.PersianToEnglish(membersView.Rows[membersView.SelectedCells[0].RowIndex].Cells[membersView.SelectedCells[0].ColumnIndex + membersView.ColumnCount-2].Value.ToString()));
            string p = "";
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                if (reader.Read())
